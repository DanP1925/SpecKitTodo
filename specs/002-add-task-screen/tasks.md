...
- [x] T015 [US1] Write failing `AddTaskScreenTest.kt` in `app/src/androidTest/kotlin/com/example/taskprioritylist/presentation/addtask/` — `@HiltAndroidTest`; use `ComposeTestRule`; inject `FakeTaskRepository`; test cases: (1) save with valid title → task saved + screen closed, (2) save with blank title → title error shown, (3) back with dirty form → discard dialog shown, (4) back with clean form → no dialog, (5) save with Important=on + Urgent=off → after navigation back, task appears in tier-2 using `TaskListRobot.assertTaskHasPriorityTier()`, (6) back with dirty form → tap "Keep Editing" → dialog dismissed + `AddTaskRobot.assertFormIntact(title)`, (7) back with dirty form → tap "Discard" → screen closes + task not saved (`TaskListRobot.assertTaskDoesNotExist`), (8) scroll task list to bottom → navigate to AddTask → save → scroll position preserved (`TaskListRobot.assertTaskDoesNotExist` for first item + `assertTaskIsDisplayed` for last); delegate all interactions to `AddTaskRobot` and `TaskListRobot` (depends on T014)
...
- T020 depends on T007 (foundational), T019
...
Task: "Create AddTaskUiState + TitleValidationError"  # T007
